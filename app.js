...
// IPN Domain Info
const ipnDomainInfo = {
    1: { code: "Di", fullName: "Dorsal IPN, subdomain i", cholinergic: false, color: "#1f77b4" },
    2: { code: "Dii", fullName: "Dorsal IPN, subdomain ii", cholinergic: false, color: "#aec7e8" },
    3: { code: "I", fullName: "Intermediate IPN", cholinergic: false, color: "#ff7f0e" },
    4: { code: "Vi", fullName: "Ventral IPN, subdomain i", cholinergic: true, color: "#2ca02c" },
    5: { code: "Vii", fullName: "Ventral IPN, subdomain ii", cholinergic: true, color: "#98df8a" },
    6: { code: "Viii", fullName: "Ventral IPN, subdomain iii", cholinergic: true, color: "#d62728" }
};

// Cluster colors (matching Seurat default palette)
const clusterColors = [
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    fetch('umap_coordinates.csv')
        ...
        .then(csv => {
            ...
            document.getElementById('umapPlot').innerHTML = '<div class="loading">Click a color option to render UMAP</div>';
            renderUMAP();
        })
        .catch(e => {
...
function showClusterResult(clusterName) {
    ...
    if (cluster) {
        ...
        resultDiv.innerHTML = `
            ...
            <p><strong>Dominant target:</strong> ${dom.fullName} (${dom.code})</p>
            ...
            <p><strong>Cholinergic:</strong> ${dom.cholinergic ? 'Yes' : 'No'}</p>
...
function showIPNResult(ipnNum) {
    ...
    if (matches.length === 0) {
        html += '<p>No clusters have this as their dominant target.</p>';
    } else {
...
function showIPNGeneResult(ipnNum) {
    ...
    if (matches.length === 0) {
        html += '<p>No genes have this as their dominant target.</p>';
    } else {
    ...
    if (colorBy === 'cluster') {
        ...
        domains.forEach(ipn => {
            ...
            traces.push({
                ...
                mode: 'markers',
                type: 'scattergl',
                name: `${info.code} (${info.cholinergic ? 'chol' : 'non-chol'})`,
                marker: {
                    size: 4,
                    color: info.color,
                    opacity: 0.7
...
document.getElementById('umapColorSelect').addEventListener('change', function() {
    const val = this.value;
    document.getElementById('geneSelectContainer').style.display = val === 'gene' ? 'block' : 'none';
    
    if (val === 'cluster' || val === 'ipn') {
        renderUMAP(val);
    }
});

document.getElementById('umapGeneInput').addEventListener('keypress', function(e) {
    if (e.key === 'Enter') {
